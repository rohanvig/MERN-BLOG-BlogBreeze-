...
export const getposts = async (req, res, next) => {
  try {
    ...
    // Total post counts (considering the filter applied)
...
export const premiumblogs = async (req, res, next) => {
  try {
    const blogs = await Post.find({ isPremium: true }).sort({ createdAt: -1 });
    console.log(blogs);

    if (!blogs.length) {
      return res.status(404).json({ message: "No premium blogs found" });
    }
